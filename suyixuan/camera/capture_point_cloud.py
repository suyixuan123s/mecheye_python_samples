...
while True:
    ...
    # camera_coordinate = rs.rs2_deproject_pixel_to_point(intrin=depth_intrin, pixel=[x, y], depth=distance)
    # depth_intrin 从上一步获取
    # x 像素点的x
    # y 像素点的y
    # distance 上一步计算的真实距离（输入的dis与输出的距离是一样的，改变的只是x与y



    distance = aligned_depth_frame.get_distance(x, y)  # 深度单位是m
    print('===============方法1：二维映射三维函数=============')
    print('depth: ', distance)  # ('depth: ', 2.502000093460083)

    camera_coordinate = rs.rs2_deproject_pixel_to_point(depth_intrin, depth_pixel, distance)
    print('camera_coordinate: ',camera_coordinate)
    # ('camera_coordinate: ', [-0.022640999406576157, -0.03151676058769226, 2.5230000019073486])
    ...
    i = y * 640 + x
    print('test_output_point', [float(vtx[i][0]), float(vtx[i][1]), float(
        vtx[i][2])])  # ('test_output_point', [-0.022542288526892662, -0.031379349529743195, 2.51200008392334])
    ...
    cv2.putText(img_color, "distance:" + str(img_depth[320, 240]) + " m", (40, 40), cv2.FONT_HERSHEY_SIMPLEX, 1.2,
                [255, 0, 255])
    cv2.putText(img_color, "X:" + str(float(vtx[y][x][0][0])) + " m", (80, 80), cv2.FONT_HERSHEY_SIMPLEX, 1.2,
                [255, 0, 255])
    cv2.putText(img_color, "Y:" + str(float(vtx[y][x][0][1])) + " m", (80, 120), cv2.FONT_HERSHEY_SIMPLEX, 1.2,
                [255, 0, 255])
    cv2.putText(img_color, "Z:" + str(float(vtx[y][x][0][2])) + " m", (80, 160), cv2.FONT_HERSHEY_SIMPLEX, 1.2,
                [255, 0, 255])
