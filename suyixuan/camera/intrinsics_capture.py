...
def get_aligned_images():
    ...
    depth_intrinsic = aligned_depth_frame.profile.as_video_stream_profile().intrinsics  # 获取深度参数（像素坐标系转相机坐标系会用到）

    fx = color_intrinsic.fx  # 焦距x
    fy = color_intrinsic.fy  # 焦距y
    cx = color_intrinsic.ppx  # 主点x
    cy = color_intrinsic.ppy  # 主点y
    distortion = color_intrinsic.model  # 畸变模型
    coeffs = color_intrinsic.coeffs  # 畸变系数
    ...
    return color_intrinsic, depth_intrinsic, color_image, depth_image, aligned_depth_frame


if __name__ == "__main__":
    while 1:
        color_intrinsic, depth_intrinsic, rgb, depth, aligned_depth_frame = get_aligned_images()  # 获取对齐的图像与相机内参
        # 定义需要得到真实三维信息的像素点（x, y)，本例程以中心点为例
        print("============")
        print(aligned_depth_frame)
        x = 320
        y = 240
        distance = aligned_depth_frame.get_distance(x, y)  # （x, y)点的真实深度值
        print("distance: ", distance)
        camera_coordinate = rs.rs2_deproject_pixel_to_point(depth_intrinsic, [x, y],
                                                            distance)  # （x, y)点在相机坐标系下的真实值，为一个三维向量。其中camera_coordinate[2]仍为dis，camera_coordinate[0]和camera_coordinate[1]为相机坐标系下的xy真实距离。
