...
class TransformPointCloud(object):
    ...
    def get_transformed_textured_point_cloud(self):
        ...
        transformation = get_transformation_params(self.camera)
        transformed_textured_point_cloud = transform_textured_point_cloud(transformation,
                                                                          self.frame_all_2d_3d.get_textured_point_cloud())
        textured_point_cloud_file = "dataset/TexturedPointCloud.ply"
    ...
    def get_transformed_point_cloud_with_normals(self):
        ...
        transformation = get_transformation_params(self.camera)
        frmae_3d = Frame3D(self.frame_all_2d_3d.frame_3d())
        transformed_point_cloud_with_normals = transform_point_cloud_with_normals(transformation,
                                                                                  frmae_3d.get_untextured_point_cloud())
        point_cloud_with_normals_file = "PointCloudWithNormals.ply"
        show_error(
            Frame3D.save_point_cloud_with_normals(transformed_point_cloud_with_normals, FileFormat_PLY,
                                                  point_cloud_with_normals_file, False))
    ...
    def get_transformed_textured_point_cloud_with_normals(self):
        ...
        transformation = get_transformation_params(self.camera)
        transformed_textured_point_cloud_with_normals = transform_textured_point_cloud_with_normals(transformation,
                                                                                                    self.frame_all_2d_3d.get_textured_point_cloud())
        textured_point_cloud_with_normals_file = "TexturedPointCloudWithNormals.ply"
        show_error(
            Frame2DAnd3D.save_point_cloud_with_normals(transformed_textured_point_cloud_with_normals, FileFormat_PLY,
    ...
    def main(self):
        if find_and_connect(self.camera):
            ...
            if (transformation.__is__valid__() == False):
                print(
                    "Transformation parameters are not set. Please configure the transformation parameters using the custom coordinate system tool in the client.")
